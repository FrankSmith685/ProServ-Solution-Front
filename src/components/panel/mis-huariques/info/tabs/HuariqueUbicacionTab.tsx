...
export const HuariqueUbicacionTab = ({
  form,
  onChange,
  ...
  const isManualAddressChange = useRef(false);
  const hasInitializedUbigeo = useRef(false);
  
  
  const [addressOptions, setAddressOptions] = useState< { label: string; lat: number; lng: number }[]>([]);
  const [isSearchingAddress, setIsSearchingAddress] = useState(false);

  const {categories, setService, service, user} = useAppState();

  // Asigna coordenadas iniciales al formulario si aún no existen
    useEffect(() => {
      if(user?.tieneEmpresa) return;
      if (!lat || !lng) return;
  ...
  useEffect(() => {
      ...
      getProvincias(form.departamento).then(provs => {
          setProvincias(provs);
      });
  }, [form.departamento]);
  ...
  const distritoOptions = distritos.map(dist => ({
      value: dist,
      label: dist,
  }));
  ...
  <div className="space-y-6 md:space-y-8">
    <div className="space-y-1">
      ...
      <p className="text-xs md:text-sm text-gray-600">
        Esta información permitirá mostrar tu huarique en el mapa y ayudar
        a los usuarios a llegar fácilmente.
      </p>
    </div>

    <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 md:p-6 space-y-4 md:space-y-5">
      <p className="text-xs md:text-sm text-gray-600">
        Indica la ubicación exacta de tu huarique para que los usuarios
        puedan encontrarlo fácilmente.
      </p>
      <p className="text-[11px] md:text-xs text-gray-500">
        • Asegúrate de que la dirección y el marcador sean correctos
      </p>
    ...
    <div className="rounded-2xl overflow-hidden">
      <HuariqueMapPicker
        ...
        lng={form.lng}
        subcategoria={(() => {
          const sub = categoriaSeleccionada?.Subcategorias.find(
            s => String(s.SUBC_Id) === form.subcategoria
          );

          if (!sub || !categoriaSeleccionada) return null;

          return {
            cod_subcategoria: sub.SUBC_Id,
            descripcion: sub.SUBC_Nombre,
            cod_categoria: categoriaSeleccionada.CATE_Id, // 👈 AQUÍ
          };
        })()}
