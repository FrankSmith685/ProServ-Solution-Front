...
const ProyectosHero: FC = () => {
  const { siteConfig, projects } = useAppState();

  const content = useMemo(() => {
    const totalProjects = Array.isArray(projects)
      ? projects.filter((item) => item?.activo !== false).length
      : 0;

    return {
      badge: "Nuestros proyectos",
      title: "Proyectos ejecutados con calidad y resultados reales",
      description:
        safeText(siteConfig?.site_description) ||
        "Conoce algunos de nuestros proyectos más destacados, desarrollados con planificación, experiencia y enfoque en resultados.",
      totalProjects,
    };
  }, [siteConfig, projects]);
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          >
            <div className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/8 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-white/88">
              ...
              {content.totalProjects} proyectos disponibles
            </div>
          </motion.div>
